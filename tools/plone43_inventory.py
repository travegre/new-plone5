...
Run in the Plone 4.3 buildout with::

    bin/instance run src/plone43_inventory.py --output-dir=/plone/instance/src

This is deliberately a single file.  It accepts only the launcher argv
shapes used by the Plone 4.3/Zope 2.13 ``instance run`` command and the
actual inventory option ``--output-dir``.  It never writes to ZODB objects.
"""
...
def scalar(value):
    ...
    try:
        if hasattr(value, 'absolute_url_path'):
            return {'path': path(value), 'uid': uid(value), 'class': dotted(value)}
    except Exception:
        pass
    return text(value)
...
class Context(object):
    ...
    def error(self, operation, obj=None, field=None, exception=None, message=None):
        ...
        if obj is not None:
            for key, fn in (('path', lambda: path(obj)),
                            ('portal_type', lambda: ptype(obj)),
...
def at_schema(obj, ctx):
    """Inspect Archetypes only when the runtime object exposes Schema()."""
    try:
    ...
    result = {'archetypes': True, 'available': True,
              'schema_class': dotted(schema_obj), 'fields': []}
...
def field_info(field, obj, ctx):
    name = safe(ctx, 'field.name', field.getName, obj=obj,
                default=None)
    ...
    for attr in ('widget', 'widgetFactory', 'enforceVocabulary'):
        try:
            ...
            result['errors'].append(ctx.error('field.%s' % attr, obj=obj,
                                              field=name, exception=exc))
    result['vocabulary'] = field_vocabulary(field, obj, ctx, name)
    return result


def field_vocabulary(field, obj, ctx, name):
    result = {}
...
def field_values(obj, schema_data, ctx):
    result = {}
    if not schema_data.get('available'):
        return result
    try:
    ...
    for field in list(fields)[:MAX_FIELDS]:
        ...
        try:
            ...
            try:
                ...
                continue
        result[name] = {'value': scalar(value)}
        try:
            result[name]['content_type'] = text(field.getContentType(obj))
        except Exception as exc:
            ctx.error('field.value.content_type', obj=obj, field=name, exception=exc)
    return result


def binaries(obj, schema_data, ctx):
    result = []
    if not schema_data.get('available'):
        return result
    try:
...
def local_roles(obj, ctx):
    ...
    blocked = safe(ctx, 'local_role_block', obj.get_local_role_block, obj=obj, default=None)
    try:
...
def references(obj, catalog, ctx):
    ...
    try:
        for target in obj.getRefs():
            item = {'path': path(target), 'uid': uid(target), 'class': dotted(target)}
        ...
        try:
            for ref in catalog.getBackReferences(obj):
                try:
                    ...
                    target = None
                result['incoming' if target else 'broken_incoming'].append(
                    {'uid': uid(ref), 'path': path(target) if target else None})
        except Exception as exc:
...
def inspect_object(obj, wf, refs, ctx):
    schema_data = at_schema(obj, ctx)
    return {
        'path': safe(ctx, 'object.path', lambda: path(obj), obj=obj),
        'id': safe(ctx, 'object.id', obj.getId, obj=obj, default=getattr(obj, 'id', None)),
        'title': safe(ctx, 'object.title', obj.Title, obj=obj, default=getattr(obj, 'title', None)),
        'portal_type': ptype(obj), 'class': dotted(obj),
        'class_hierarchy': hierarchy(obj), 'interfaces': interfaces(obj),
        'archetypes_schema': schema_data,
        'field_values': field_values(obj, schema_data, ctx),
        'stored_attributes': stored_attributes(obj, ctx),
        'workflow': workflow(obj, wf, ctx),
        'local_roles': local_roles(obj, ctx),
        'permissions': permissions(obj, ctx),
        'references': references(obj, refs, ctx),
        'binaries': binaries(obj, schema_data, ctx),
        'folderish': safe(ctx, 'object.folderish', lambda: IFolderish.providedBy(obj), obj=obj, default=False),
    }
...
def type_definition(site, pt, ctx):
    try:
        ...
        ctx.error('portal_type.definition', obj=site, message=pt, exception=exc)
        return None
    if info is None:
        return None
    result = {'id': pt, 'title': text(getattr(info, 'title', None)),
              ...
              'aliases': dict(getattr(info, 'aliases', {}) or {}), 'actions': [],
              'workflow_chain': [], 'permissions': {}}
    try:
        result['actions'] = [scalar(a) for a in info.listActions()]
    except Exception as exc:
        ctx.error('portal_type.actions', obj=site, message=pt, exception=exc)
    try:
        result['workflow_chain'] = list(site.portal_workflow.getChainForPortalType(pt) or ())
    except Exception as exc:
        ctx.error('portal_type.workflow_chain', obj=site, message=pt, exception=exc)
    try:
        result['permissions'] = dict(info.getPermissionMap())
    except Exception as exc:
        ctx.error('portal_type.permissions', obj=site, message=pt, exception=exc)
    return result
...
def pfg_dump(obj, ctx, depth=0):
    ...
    try:
        if IFolderish.providedBy(obj):
            for child in list(obj.objectValues() or [])[:500]:
...
def site_inventory(site, name, label, errors):
    ctx = Context(name)
    wf = safe(ctx, 'site.portal_workflow', lambda: site.portal_workflow, obj=site)
    refs = safe(ctx, 'site.reference_catalog', lambda: site.reference_catalog, obj=site)
    counts = Counter(); classes = defaultdict(set); samples = defaultdict(list)
    pfgs = []; unknown = []; chains = defaultdict(Counter); role_counts = Counter()
    binary_counts = Counter(); binary_bytes = Counter(); total = 0
    try:
        for obj in walk(site, ctx):
            total += 1
            try:
                pt = ptype(obj) or '<no portal_type>'
                counts[pt] += 1; classes[pt].add(dotted(obj))
            except Exception as exc:
                ctx.error('object.classification', obj=obj, exception=exc)
                pt = '<classification error>'; counts[pt] += 1
            if len(samples[pt]) < MAX_SAMPLES:
                try:
                    samples[pt].append(inspect_object(obj, wf, refs, ctx))
                except Exception as exc:
                    ctx.error('object.summary', obj=obj, exception=exc)
                    samples[pt].append({'path': path(obj), 'portal_type': ptype(obj),
            ...
            try:
                for wid in workflow(obj, wf, ctx).get('chain', []):
                    chains[pt][wid] += 1
            except Exception as exc:
            ...
            if len(samples[pt]) <= MAX_SAMPLES:
                try:
                    for b in samples[pt][-1].get('binaries', []):
                        binary_counts[b['type']] += 1
                        binary_bytes[b['type']] += b.get('size') or 0
                except Exception as exc:
    ...
    for pt in sorted(counts):
        types[pt] = {'count': counts[pt],
                     'classes': sorted(x for x in classes[pt] if x),
                     'definition': type_definition(site, pt, ctx),
                     'workflow_assignments': dict(chains[pt]),
                     'samples': samples[pt]}
    errors.extend(ctx.errors)
    return {'site': name, 'path': safe(ctx, 'site.path', site.absolute_url_path, obj=site),
            ...
            'objects_with_local_roles': dict(role_counts), 'class': dotted(site),
...
def global_inventory(app, errors):
    ...
    try:
        for info in app.portal_types.objectValues():
            try:
                result['portal_types'].append({'id': getattr(info, 'id', None),
                    'title': text(getattr(info, 'title', None)),
                    'factory': text(getattr(info, 'factory', None)),
                    'klass': text(getattr(info, 'klass', None))})
            except Exception as exc:
                errors.append({'site': None, 'path': '/portal_types', 'portal_type': None,
                    'class': dotted(info), 'operation': 'global.portal_type', 'field': None,
    ...
    try:
        for definition in app.portal_workflow.objectValues():
            try:
                states = [getattr(s, 'id', None) for s in definition.states.objectValues()]
            except Exception as exc:
                states = []
                errors.append({'site': None, 'path': None, 'portal_type': None,
                    'class': dotted(definition), 'operation': 'global.workflow.states',
                    'field': None, 'exception_type': exc.__class__.__name__, 'message': text(exc)})
...
def markdown(data):
    lines = ['# Plone 4.3 Read-only Database Inventory', '',
             'Generated by `tools/plone43_inventory.py`. Representative values are bounded.', '',
             '## Runtime', '', '- Generated: `%s`' % data['generated_at'],
             '- Python: `%s`' % data['runtime']['python'].replace('\n', ' '),
             '- Zope: `%s`' % data['runtime'].get('zope'),
             '- Plone: `%s`' % data['runtime'].get('plone'),
             '- Read-only mode: `%s`' % data['read_only'], '',
             '## Inspection errors', '', '**Total:** %s' % len(data['errors']), '']
    if data['errors']:
        lines.extend(['| Site | Path | Portal type | Class | Operation | Field | Exception | Message |',
                      '|---|---|---|---|---|---|---|---|'])
        for e in data['errors'][:5000]:
            vals = [text(e.get(k) or '', 500).replace('|', '\\|').replace('\n', ' ')
                    for k in ('site', 'path', 'portal_type', 'class', 'operation',
    ...
    for site in data['sites']:
        ...
        for pt, info in sorted(site['portal_types'].items()):
            lines.append('| `%s` | %s | %s | %s |' % (pt, info['count'],
        ...
        lines.extend(['```json', json.dumps(site['unknown_objects'], ensure_ascii=False,
            indent=2, sort_keys=True), '```'] if site['unknown_objects'] else ['None detected.'])
...
def parse_inventory_args(argv, script_name):
    """Extract only arguments belonging to this script from Zope's argv."""
    argv = list(argv)
    expected = os.path.basename(script_name)
    positions = [i for i, value in enumerate(argv)
                 if os.path.basename(value) == expected]
    if not positions:
        raise SystemExit('Unexpected Plone/Zope run argv shape: %r' % argv)
    # The last occurrence is the actual script name; preceding values are
    # Python/Zope launcher bookkeeping, including a possible -c.
    ...
    for name, site_path, label in SITES:
        try:
            ...
            errors.append({'site': name, 'path': site_path, 'portal_type': None, 'class': None,
                'operation': 'site.open', 'field': None, 'exception_type': exc.__class__.__name__,
                'message': text(exc), 'traceback': traceback.format_exc()})
