...
sites.  Reads ``pfg-easyform.jsonl`` produced by
``plone43_export_easyforms.py`` and creates/updates EasyForm objects at the
same source-relative paths.
"""
import json
import os
import sys
...
def apply_record(obj, record):
    # These are the canonical EasyForm model strings generated by EasyForm's
    # own PFG migration helpers on the old site.
    ...
    for name in (
        ...
        if name in record:
            try:
                ...
                # EasyForm versions differ slightly in optional thank-you
                # fields. Required model fields above are never ignored.
                pass
    obj.reindexObject()
