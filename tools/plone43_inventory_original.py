...
Run from a Plone 4.3 buildout with:
    bin/instance run src/plone43_inventory.py --output-dir=/plone/instance/src

The Zope 2.13 ``run`` command may leave launcher bookkeeping (including
``-c`` and/or the script name) in sys.argv.  Only the documented launcher
shapes are accepted; unrelated arguments are rejected.
"""
...
def hierarchy(obj):
    ...
    while cls is not None and cls not in seen:
        seen.add(cls)
        result.append('%s.%s' % (getattr(cls, '__module__', '<unknown>'),
                                 getattr(cls, '__name__', '<unknown>')))
...
def archetypes_schema(obj, ctx):
    """Return the real AT schema when the runtime object exposes Schema()."""
    try:
    ...
    for field in list(fields)[:MAX_FIELDS]:
        result['fields'].append(schema_field(field, obj, ctx))
    return result
...
def vocabulary(field, obj, ctx):
    ...
    for method_name in ('Vocabulary', 'vocabulary', 'getVocabulary'):
        try:
            ...
            ctx.error('field.vocabulary.capability', obj=obj, field=name,
                      exception=exc)
            continue
        ...
        try:
            ...
            result['error'] = {'exception_type': exc.__class__.__name__,
                               'message': text(exc)}
...
def binaries(obj, at_schema, ctx):
    ...
    for field in list(fields):
        ...
        try:
            ...
            filename = None
        result.append({'field': name, 'type': field_class, 'size': size,
                       'filename': filename})
    return result


def local_roles(obj, ctx):
    roles = safe_call(ctx, 'local_roles', lambda: obj.get_local_roles(),
                      obj=obj, default={})
    blocked = safe_call(ctx, 'local_role_block',
                        lambda: obj.get_local_role_block(), obj=obj)
    try:
...
def references(obj, reference_catalog, ctx):
    result = {'outgoing': [], 'incoming': [],
              'broken_outgoing': [], 'broken_incoming': []}
    try:
        refs = obj.getRefs()
        for target in refs:
            item = {'path': path(target), 'uid': safe_uid(target),
                    'class': dotted(target)}
...
def summary(obj, wf, reference_catalog, ctx):
    ...
    return {
        ...
        'id': safe_call(ctx, 'object.id', lambda: obj.getId(), obj=obj,
                        default=getattr(obj, 'id', None)),
        'title': safe_call(ctx, 'object.title', lambda: obj.Title(), obj=obj,
                           default=safe_call(ctx, 'object.title_attribute',
                                             lambda: obj.title, obj=obj)),
        'portal_type': ptype(obj),
        'class': dotted(obj),
        'class_hierarchy': hierarchy(obj),
        'interfaces': interfaces(obj),
        'archetypes_schema': at_schema,
        'field_values': field_values(obj, at_schema, ctx),
        'stored_attributes': stored(obj, ctx),
        ...
        'folderish': safe_call(ctx, 'object.folderish',
...
def walk(root, ctx):
    ...
    while stack:
        ...
        yield obj
        try:
            folderish = IFolderish.providedBy(obj)
        except Exception as exc:
...
def type_definition(site, portal_type, ctx):
    try:
        ...
        ctx.error('portal_type.definition', obj=site, message=portal_type,
                  exception=exc)
        return None
    ...
    try:
        ...
        ctx.error('portal_type.actions', obj=site, message=portal_type,
                  exception=exc)
    try:
        result['workflow_chain'] = list(
            site.portal_workflow.getChainForPortalType(portal_type) or ())
    except Exception as exc:
        ctx.error('portal_type.workflow_chain', obj=site,
                  message=portal_type, exception=exc)
    try:
        result['permissions'] = dict(type_info.getPermissionMap())
    except Exception as exc:
        ctx.error('portal_type.permissions', obj=site, message=portal_type,
                  exception=exc)
    return result
...
def pfg(obj, ctx, depth=0):
    ...
    result = {
        ...
        'id': safe_call(ctx, 'pfg.id', lambda: obj.getId(), obj=obj,
                        default=getattr(obj, 'id', None)),
...
def site_inventory(site, name, label, global_errors):
    ...
    wf = safe_call(ctx, 'site.portal_workflow', lambda: site.portal_workflow,
                   obj=site)
    reference_catalog = safe_call(ctx, 'site.reference_catalog',
                                  lambda: site.reference_catalog, obj=site)
    counts = Counter()
    classes = defaultdict(set)
    samples = defaultdict(list)
    pfg_objects = []
    unknown = []
    workflow_assignments = defaultdict(Counter)
    local_role_counts = Counter()
    binary_counts = Counter()
    binary_bytes = Counter()
    total = 0

    try:
        objects = walk(site, ctx)
        for obj in objects:
            total += 1
            try:
            ...
            if len(samples[portal_type]) < MAX_SAMPLES:
                try:
                    samples[portal_type].append(
                        summary(obj, wf, reference_catalog, ctx))
                except Exception as exc:
                    ctx.error('object.summary', obj=obj, exception=exc)
                    samples[portal_type].append({
                        'path': safe_call(ctx, 'object.error_path',
                                          lambda: path(obj), obj=obj),
                        'portal_type': ptype(obj),
                        'class': dotted(obj),
            ...
            try:
                current_sample = samples[portal_type][-1]
                if current_sample.get('path') == path(obj):
                    for binary in current_sample.get('binaries', []):
    ...
    for portal_type in sorted(counts):
        types[portal_type] = {
            'count': counts[portal_type],
            'classes': sorted(x for x in classes[portal_type] if x),
    ...
    return {
        'site': name,
        'path': safe_call(ctx, 'site.path', lambda: site.absolute_url_path(),
                          obj=site),
        'title': label,
        'objects_walked': total,
        'portal_types': types,
        'pfg_objects': pfg_objects,
        'unknown_objects': unknown,
        'binary_summary': {
            'counts': dict(binary_counts),
            'bytes': dict(binary_bytes),
            'note': 'Binary details are complete for representative samples; aggregate counts are not inferred for unsampled objects.',
        },
        'objects_with_local_roles': dict(local_role_counts),
        'class': dotted(site),
        'class_hierarchy': hierarchy(site),
...
def global_inventory(app, errors):
        ...
        for type_info in app.portal_types.objectValues():
            try:
                ...
                errors.append({'site': None, 'path': '/portal_types',
                               'portal_type': None, 'class': dotted(type_info),
                               'operation': 'global.portal_type', 'field': None,
                               'exception_type': exc.__class__.__name__,
                               'message': text(exc)})
    except Exception as exc:
        errors.append({'site': None, 'path': '/', 'portal_type': None,
                       'class': dotted(app), 'operation': 'global.portal_types',
    ...
    try:
        ...
        for definition in workflow_tool.objectValues():
            states = []
            try:
                states = [getattr(state, 'id', None)
                          for state in definition.states.objectValues()]
            except Exception as exc:
                errors.append({'site': None, 'path': None, 'portal_type': None,
                               'class': dotted(definition),
                               'operation': 'global.workflow.states',
                               'field': None,
                               'exception_type': exc.__class__.__name__,
                               'message': text(exc)})
...
def markdown(data):
    ...
    if data['errors']:
        ...
        for error in data['errors'][:5000]:
            vals = [error.get(k) or '' for k in
                    ('site', 'path', 'portal_type', 'class', 'operation',
    ...
    for site in data['sites']:
        ...
        for portal_type, info in sorted(site['portal_types'].items()):
            lines.append('| `%s` | %s | %s | %s |' % (
                portal_type, info['count'], '<br>'.join(info['classes']),
        ...
        for portal_type, info in sorted(site['portal_types'].items()):
            for sample in info['samples']:
                lines.extend([
                    ...
                    (portal_type, sample.get('path')), '', '```json',
                    json.dumps(sample, ensure_ascii=False, indent=2,
                               sort_keys=True), '```', '</details>', ''])
    return '\n'.join(lines) + '\n'


def _script_basename(value):
    try:
...
def parse_inventory_args(argv, script_name):
    """Parse arguments from the real Plone 4.3 ``instance run`` wrapper.

    Depending on the Zope 2.13 recipe/wrapper, sys.argv can contain one of:
    ...
    Only these exact launcher forms are accepted.  In particular, arbitrary
    unknown options are never silently ignored.
    """
    argv = list(argv)
    if not argv:
        raise SystemExit('Unable to determine inventory script arguments.')
    expected = _script_basename(script_name)
    first = _script_basename(argv[0])

    if first == expected:
        args = argv[1:]
        if args and _script_basename(args[0]) == expected:
            args = args[1:]
        elif args and args[0] == '-c':
            args = args[1:]
    elif argv[0] == '-c' and len(argv) > 1 and _script_basename(argv[1]) == expected:
        args = argv[2:]
    else:
...
def parse_output_dir(args):
    """Parse inventory options; reject every unknown argument."""
    if not args:
...
def run(app, outdir):
    ...
    result = {
        ...
        'global': global_inventory(app, errors),
        'sites': [],
        'errors': errors,
    }
    try:
        ...
        errors.append({'site': None, 'path': None, 'portal_type': None,
                       'class': None, 'operation': 'runtime.zope_version',
                       'field': None, 'exception_type': exc.__class__.__name__,
                       'message': text(exc)})
    try:
        ...
        errors.append({'site': None, 'path': None, 'portal_type': None,
                       'class': None, 'operation': 'runtime.plone_version',
                       'field': None, 'exception_type': exc.__class__.__name__,
                       'message': text(exc)})
